var WxRenderer = function (opts) {	//最新版
  this.opts = opts
  ...
  this.buildFootnotes = function () {
    var footnoteArray = footnotes.map(function (x) {
      if (x[1] === x[2]) {
        return '<code style="font-size: 90%; opacity: 0.6;">[' + x[0] + ']</code><i>'  + x[1] +'<br/><br/></i>'
      }
      return '<code style="font-size: 90%; opacity: 0.6;">[' + x[0] + ']</code> ' + x[1] + '<br/><i>'  + x[2] +'<br/><br/></i>'
    })
    return '<h2 ' + S('h2') + '>相关链接</h2><p ' + S('footnotes') + '>'  + footnoteArray.join('\n') + '</p>'
  }
...
//        text = segments.join('•❥▪✪⊹☾');
    ...
    return '<strong ' + S('strong') + '>'  + '「'+ text + '」'+'</strong>'; 	//加粗字体加「」
//      return '<strong ' + S('strong') + '>' + text + '</strong>'; 	//加粗字体默认样式
    }
    ...
    renderer.hr = function(){
      return '<hr style="border-style: solid;border-top: 1px;margin: 20px 10px;border-color: rgba(255,76,64,0.8);-webkit-transform-origin: 0 0;-webkit-transform: scale(1, 0.5);transform-origin: 0 0;transform: scale(1, 0.5);">';
    }
    return renderer
  }
}